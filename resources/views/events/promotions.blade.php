...
<div class="container py-4">
    <!-- Bannière de recherche spécifique aux promotions -->
    <div class="search-banner mb-5" style="background-color: var(--bleu-nuit); border-radius: var(--border-radius);">
        <div class="p-4 p-md-5 text-white">
            <div class="row align-items-center">
                <div class="col-md-8">
                    <h2 class="mb-3">Événements en promotion</h2>
                    <p class="mb-0">Découvrez nos meilleures offres pour des expériences inoubliables</p>
                </div>
            ...
            <div class="card border-0 shadow-sm mb-4 filter-card">
                <div class="card-body p-4">
                    ...
                    <div class="filter-section mb-4">
                        ...
                        <div class="filter-content">
                            <div class="d-flex flex-column gap-2">
                                ...
                                <a href="{{ request()->fullUrlWithQuery(['ville' => $ville]) }}"
                    ...
                    <div class="sticky-bottom pt-3">
                        <a href="{{ route('events.promotions') }}" class="btn btn-outline-primary w-100 d-flex align-items-center justify-content-center">
        ...
        <div class="col-lg-9">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h2 class="h5 mb-0">
                    ...
                    @else
                        Événements en promotion ({{ $events->total() }})
                    @endif
                </h2>
            ...
            @if($events->count() > 0)
                ...
                <div class="list-view" style="display: none;">
                    @foreach($events as $event)
                    @php
                        $promoTickets = $event->tickets->filter(function($ticket) {
                            return $ticket->isPromoActive();
                        });
                        $minPromoPrice = $promoTickets->min('montant_promotionnel');
                        ...
                        <div class="row g-0 h-100">
                            <div class="col-md-4">
                                <div class="position-relative h-100" style="min-height: 200px;">
                                    ...
                                    <!-- Badges (Promo, État, Type) -->
                                    <div class="position-absolute top-0 start-0 m-2">
                                        <span class="badge bg-danger">Promo</span>
                                        @if($event->etat == 'En cours')
                                            <span class="badge mt-1" style="background-color: var(--bleu-nuit);">{{ $event->etat }}</span>
                                        @else
                                            <span class="badge mt-1 bg-secondary">{{ $event->etat }}</span>
                                        @endif
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <div class="d-flex align-items-center gap-3">
                                            ...
                                            @if($minPromoPrice)
                                                <span class="badge bg-danger">
                                                    <del class="text-white">{{ number_format($promoTickets->min('prix'), 0) }} XAF</del>
                                                    {{ number_format($minPromoPrice, 0) }} XAF
                                                </span>
                                            @endif
                                        </div>
                                        @if($hasTickets)
            ...
            @else
                <div class="alert alert-info">
                    <i class="fas fa-info-circle me-2"></i> Aucun événement en promotion ne correspond à vos critères de recherche.
                    <a href="{{ route('events.promotions') }}" class="alert-link">Réinitialiser les filtres</a>
                </div>
            @endif
        </div>
    </div>
...
/* Ajout spécifique pour les promotions */
/* Remplacer le rouge par le bleu et assurer le contraste */
.badge.bg-danger,
.text-bg-danger {
    background-color: var(--bleu-nuit) !important;
    color: #ffffff !important;
...
document.addEventListener('DOMContentLoaded', function() {
    // Masquer skeleton et afficher contenu réel
    setTimeout(() => {
        document.querySelector('.skeleton-content').style.display = 'none';
        document.querySelector('.real-content').style.display = 'flex';
    ...
    // Initialisation du toggle des vues
    ...
    // Initialisation des filtres accordéon
    function initFilters() {
        ...
        // Ouvrir les sections avec des filtres actifs
        document.querySelectorAll('.filter-chip.active').forEach(chip => {
            const section = chip.closest('.filter-section');
            if (section) {
                section.classList.add('active');
                const content = section.querySelector('.filter-content');
                const icon = section.querySelector('.filter-toggle');
                if (content) {
                    content.style.maxHeight = content.scrollHeight + 'px';
                    content.style.opacity = '1';
                }
            ...
            button.addEventListener('click', async function(e) {
                ...
                try {
                    ...
                    if (data.status === 'added') {
                        ...
                        showToast('Événement ajouté aux favoris', 'success');
                    } else if (data.status === 'removed') {
                        icon.classList.remove('fas');
                        icon.classList.add('far');
                        showToast('Événement retiré des favoris', 'error');
                    }
                } catch (error) {
